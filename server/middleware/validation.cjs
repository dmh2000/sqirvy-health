const validUnits = [
  ...
  'small', 'medium', 'large', 'slice', 'piece'
];
...
function validateFoodItem(foodItem) {
  ...
  if (!foodItem.name || typeof foodItem.name !== 'string' || foodItem.name.trim().length === 0) {
  ...
  if (!foodItem.kcal || typeof foodItem.kcal !== 'number' || foodItem.kcal <= 0) {
